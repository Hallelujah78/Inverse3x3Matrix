...
public class Inverse3x3Matrix {
    ...
    // a method that takes the index of an array and a matrix side length and
    //converts the the index to a row and column value in the matrix
    private static HashMap convert1DTo2D(int index, int sideLength) {
        HashMap<String, Integer> rowAndCol = new HashMap<>();

        rowAndCol.put("row", Math.floorDiv(index, sideLength));
        rowAndCol.put("col", index % sideLength);
        return rowAndCol;
    }
    ...
    private static int calc3x3Determinant(int[][] matrix) {
        ...
        for (int i = 0; i < sideLength; i++) {
            ...
            HashMap<String, Integer> rowAndCol = convert1DTo2D(i, sideLength);
            // we get the scalar in our original array using the row and col we
            // calculated
            int scalar = matrix[rowAndCol.get("row")][rowAndCol.get("col")];
             ...
             */
            int minorIndex = 0;
            int minorMatrix[][] = new int[sideLength2x2][sideLength2x2];
            ...
            for (int j = 0; j < numElements; j++) {
                ...
                if (currRow == rowAndCol.get("row") || currCol == rowAndCol.get("col")) {
                    continue;
                }
    ...
    private static int[][] constructCofactorMatrix(int[][] matrix) {
        int numElements = matrix.length * matrix[0].length; // 9 elements in the matrix
        int sideLength = matrix.length; // 3x3 matrix
        int determinant = 0; // store the determinant sum as we calculate it
        ...
        return cofactor3x3Matrix;
    }

    private static double[][] constructInverseMatrix(int determinant, int[][] matrix) {
        ...
        for (int i = 0; i < numRows; i++) {
            ...
            // for every column
            for (int j = 0; j < numCols; j++) {
                inverseMatrix[i][j] = (matrix[i][j] * 1.0 / determinant);
            }
        }
        System.out.println("inverse matrix: " + Arrays.deepToString(inverseMatrix));
